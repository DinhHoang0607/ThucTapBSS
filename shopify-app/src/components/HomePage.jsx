...
export function HomePage() {
  ...
  const [amountProductsConst, setAmountProductsConst] =
    useState('')
  const [amountProductsFixed, setAmountProductsFixed] =
    useState('')
  ...
  const handleAmountProductConst = (value) => {
    handleConstPriceError(value)
    setAmountProductsConst(value)
  }
  ...
  const handleAmountProductFixed = (value) => {
    console.log(amountProductsFixed)
    handleFixedPriceError(value)
    setAmountProductsFixed(value)
  }
  ...
  const formatPrice = (price) => {
    ...
    // }
      ...
      let newPrice =
        price * (1 - parseFloat(constPrice) / 100)
      return (
        newPrice.toFixed(2) +
        formatCurrency(currency.shop.currencyCode)
      )
  ...
  const [errCount, setErrCount] = useState(true)
  const [
    getAllProductsByIds,
    {
    ...
    onCompleted: (data) => {
      let count = data.nodes.length
      // console.log(count);
      // console.log(amountProductsConst)
      // if (amount) {
      let Arr = data.nodes.map((product) => {
        ...
        if (count < tmp) {
          setmessToast(
            'So luong san pham da chon nho hon so luong yeu cau'
          )
          setErrCount(false)
          // setLoadingTable(false)
          return []
          ...
          axios.post('http://localhost:6767/table', {
            id: product.id,
            title,
            price,
            amountProductsConst: tmp,
          })
          console.log(product.id, title, price, tmp)
        ...
        <Layout>
          <Layout.Section>
            ...
            <Card title="Custom Prices" sectioned>
              <Stack vertical>
                <RadioButton
                  label="Discount Percent Price"
                  id="constPrice"
                  checked={priceValue === 'constPrice'}
                  onChange={handleChangePrice}
                />
                <RadioButton
                  label="Discount Amount Price"
                  id="fixedPrice"
              ...
              <span style={{ margin: '20px' }}>
                {priceValue === 'constPrice' && (
                  <TextField
                    ...
                    type="number"
                    autoComplete="off"
                    value={amountProductsConst}
                    label="Amount Product"
                    helpText={
                      <span>number greater 0</span>
                    }
                    id="priceAmount1"
                    onChange={handleAmountProductConst}
                  />
                )}
                {priceValue === 'fixedPrice' && (
                  <TextField
                    ...
                    type="number"
                    autoComplete="off"
                    value={amountProductsFixed}
                    label="Amount Product"
                    helpText={
                      <span>number greater 0</span>
                    }
                    id="priceAmount2"
                    onChange={handleAmountProductFixed}
                  />
                )}
                {priceError && (
                  <InlineError
